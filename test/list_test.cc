...
TEST(ConsC, Equality) {
  ...
  EXPECT_FALSE(pair_1->IsEqual(new ConsC(o1, Int::Get(7))));
  EXPECT_NE(pair_1, pair_2);
}

TEST(List, AllocAndDelloc) {
  Object *o1 = Int::Get(5);
  const ConsC *head = new ConsC(o1, List::kNil);
...
TEST(List, Equality) {
  ASSERT_EQ(List::kNil, List::kNil);
  Object *o1 = Int::Get(5);
  const ConsC *head_1 = new ConsC(o1, List::kNil);
  List *list_1 = new List(head_1);

  const ConsC *head_2 = new ConsC(o1, List::kNil);
  List *list_2 = new List(head_2);
  ASSERT_NE(List::kNil, list_1);
  EXPECT_TRUE(list_1->IsEqual(list_2));
  ASSERT_EQ(*list_1, *list_2);

  const ConsC *head_3 = new ConsC(Int::Get(6), List::kNil);
